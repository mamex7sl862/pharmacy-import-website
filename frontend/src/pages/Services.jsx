...
const DEFAULT_SERVICES = [
  ...
  { icon: 'thermostat', title: 'Cold Chain Logistics', desc: 'Specialized temperature-controlled storage and distribution for biologics, vaccines, and temperature-sensitive pharmaceuticals. IoT-monitored throughout the entire supply chain.', features: ['2–8°C compliance', 'IoT temperature monitoring', 'Validated cold rooms', 'Real-time tracking'], img: 'https://images.unsplash.com/photo-1559757148-5c350d0d3c56?w=800&q=80' },
  { icon: 'request_quote', title: 'RFQ & Quotation Management', desc: 'Our digital RFQ platform allows healthcare institutions to submit structured quotation requests for multiple products simultaneously. Receive formal quotations within 4–24 hours.', features: ['Multi-product RFQ', 'Digital quotation delivery', '4–24h response time', 'PDF quotation download'], img: 'https://images.unsplash.com/photo-1584308666744-24d5c474f2ae?w=800&q=80' },
...
export default function Services() {
  ...
  const STATS = [
    ...
    { value: company?.accuracy  || '99.8%',   label: 'Order Accuracy' },
  ]
            ...
            <div key={svc.title} className={`grid md:grid-cols-2 gap-16 items-center ${i % 2 !== 0 ? 'md:grid-flow-dense' : ''}`}>
              <div className={`relative ${i % 2 !== 0 ? 'md:col-start-2' : ''}`}>
                <div className="rounded-2xl overflow-hidden shadow-2xl aspect-[4/3]">
                  <img src={svc.img} alt={svc.title} className="w-full h-full object-cover hover:scale-105 transition-transform duration-700" />
                </div>
              </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-8">
            {PROCESS.map((p, i) => (
              <div key={p.step} className="relative text-center">
                {i < PROCESS.length - 1 && <div className="hidden md:block absolute top-8 left-full w-full h-0.5 bg-outline-variant/30 z-0" style={{ width: 'calc(100% - 2rem)' }} />}
                <div className="relative z-10">
                  <div className="w-16 h-16 rounded-full bg-primary text-white font-headline font-extrabold text-xl flex items-center justify-center mx-auto mb-6 shadow-lg shadow-primary/30">{p.step}</div>
          ...
          <div className="bg-primary rounded-3xl p-12 md:p-16 relative overflow-hidden">
            ...
            <div className="relative z-10 grid md:grid-cols-2 gap-10 items-center">
              <div>
                ...
                <p className="text-blue-200 text-lg leading-relaxed">Submit your first RFQ today and receive a competitive quotation from our procurement specialists within 24 hours.</p>
              </div>
              <div className="flex flex-col sm:flex-row gap-4 md:justify-end">
                <Link to="/rfq" className="bg-white text-primary px-8 py-4 rounded-xl font-headline font-bold hover:scale-[1.02] transition-all shadow-xl inline-flex items-center gap-2 justify-center">
                  Submit RFQ Now <span className="material-symbols-outlined">arrow_forward</span>
                </Link>
